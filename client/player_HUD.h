...
private:
    ...
    void render_number_string(const std::string& text, int x_pos, int y_pos, float scale, bool yellow ) {
        for (char c : text) {
            if (c >= '0' && c <= '9') {
                int index = c - '0';
...
public:
    PLayerHUD(SDL2pp::Texture& texture, SDL2pp::Renderer& renderer)
        ...
        renderer(renderer), health(0), money(0), bullets(0){}
