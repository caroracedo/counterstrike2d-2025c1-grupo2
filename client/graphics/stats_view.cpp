...
StatsView::StatsView(SDL2pp::Renderer& renderer, SoundManager& sound_manager):
        renderer(renderer),
...
void StatsView::update(const Stats& new_stats,
                       const std::unordered_map<uint16_t, PlayerType>& types) {
...
void StatsView::set_visible(bool visible) { this->visible = visible; }

void StatsView::render_team_section(PlayerType team, int start_x, int start_y) {
    std::string title =
    ...
    renderer.Copy(name_header, SDL2pp::NullOpt,
    ...
    renderer.Copy(
            deaths_header, SDL2pp::NullOpt,
            SDL2pp::Rect(deaths_x, header_y, deaths_header.GetWidth(), deaths_header.GetHeight()));
    renderer.Copy(
    ...
    for (const auto& [id, type]: player_types) {
        ...
        SDL2pp::Texture money_tex(renderer,
                                  font.RenderText_Solid("$" + std::to_string(money), text_color));

        renderer.Copy(name_tex, SDL2pp::NullOpt,
                      SDL2pp::Rect(name_x, start_y, name_tex.GetWidth(), name_tex.GetHeight()));
        renderer.Copy(kills_tex, SDL2pp::NullOpt,
...
void StatsView::reset_sounds() { sounds_played = false; }

void StatsView::render_winner_banner(WinnerTeamType winner) {
    std::string text;
    SDL_Color color;
