...
DropView::DropView(SDL2pp::Renderer& renderer, TextureManager& texture_manager):
        renderer(renderer), texture_manager(texture_manager) {
    initialize_resources();
}

void DropView::initialize_resources() {
...
void DropView::update(float x, float y, WeaponModel type) {
    this->type = type;
    this->x = x;
    this->y = y;
}

void DropView::draw(const GameCamera& camera) {
