...
InputHandler::InputHandler(GameView& game_view):
        game_view(game_view), camera(game_view.get_camera()), shop(game_view.get_shop()) {}

std::vector<ActionDTO> InputHandler::poll_actions() {
    SDL_Event event;
...
float InputHandler::calculate_angle(const std::vector<float>& player_pos) {
