...
public:
    Knife(const std::vector<uint16_t>& player_position, uint16_t _range, uint16_t _min_damage, uint16_t _max_damage, const std::vector<uint16_t>& desired_position)
        : Object(ObjectType::KNIFE, 0, player_position, 0, 0),
          range(_range), min_damage(_min_damage), max_damage(_max_damage), start_position(player_position)
    {
        ...
        end_position = {
            static_cast<uint16_t>(player_position[0] + ux * range),
    ...
    bool hits(const std::vector<uint16_t>& target_pos) const {
        ...
        float dist = std::sqrt((px - closest_x) * (px - closest_x) + (py - closest_y) * (py - closest_y));
        float dist_to_attacker = std::sqrt((closest_x - start_position[0]) * (closest_x - start_position[0]) +
    ...
    ObjectDTO get_dto() const override {
        return {ObjectType::KNIFE, start_position};
    }
};
