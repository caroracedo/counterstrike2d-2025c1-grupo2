...
std::vector<std::pair<uint16_t, uint16_t>> Game::get_cells_within_radius(
    ...
    std::set<std::shared_ptr<Object>> Game::get_adyacent_objects(const std::pair<uint16_t, uint16_t>& cell) {
        /*
    ...
    std::pair<bool, std::vector<uint16_t>> Game::_move(const Object& obj, std::vector<uint16_t> new_position) {
        /*
        ...
        std::cout << "\nIntentando mover "
                  << get_object_type(static_cast<ObjectType>(obj.get_type())) << " con ID " << obj.get_id()
                  << " desde (" << position[0] << ", " << position[1] << ")"
    ...
    std::pair<ObjectType, uint16_t> Game::collides(const Object& object,
        ...
        for (const auto& obj: objects) {
            ...
            if (overlap) {
                ...
                if (type == ObjectType::BULLET || type == ObjectType::OBSTACLE || type == ObjectType::PLAYER) {
    ...
    bool Game::update_object_in_matrix(const std::shared_ptr<Object>& obj,
        ...
        if (collider_type == ObjectType::BULLET &&
            ...
            if (!player_is_alive) { return false; }  // El jugador murió, no se mueve
        }
            ...
            bullet_vec.erase(std::remove_if(bullet_vec.begin(), bullet_vec.end(),
                                                [bullet_id_val](const std::shared_ptr<Object>& o) {
                                                    return o->get_id() == bullet_id_val && 
                                                           static_cast<ObjectType>(o->get_type()) == ObjectType::BULLET;
                                                }),
                                bullet_vec.end());
            ...
            if (!player_it->second->is_alive()) {
                ...
                player_vec.erase(std::remove_if(player_vec.begin(), player_vec.end(),
                                                [id](const std::shared_ptr<Object>& o) {
                                                    return o->get_id() == id && 
                                                           static_cast<ObjectType>(o->get_type()) == ObjectType::PLAYER;
                                                }),
                                 player_vec.end());
    ...
    std::vector<uint16_t> Game::calculate_bullet_starting_position(const std::shared_ptr<Object>& player) {
        /*
        ...
        return {static_cast<uint16_t>(position[0] + offset), static_cast<uint16_t>(position[1] + offset)};
    }
    ...
    bool Game::move(Direction direction, const uint16_t& id) {
        ...
        if (player_it != players.end()) {
            ...
            std::pair<bool, std::vector<uint16_t>> move_result = _move(static_cast<Object>(*(player_it->second)), new_position);
            if (move_result.first) {
                ...
                return update_object_in_matrix(std::static_pointer_cast<Object>(player_it->second), old_position) || old_position != move_result.second;
            }
        } else {
