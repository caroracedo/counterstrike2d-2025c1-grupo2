...
private:
    ...
    void initialize_demo_objects() {
        // Obstáculo 1 en (0, 10)
        ObjectDTO obstacle1(ObjectType::OBSTACLE, {0, 10}, 6, 6);
        objects.push_back(obstacle1);
        auto cell = get_cell_from_position(obstacle1.position);
        matrix[cell.first][cell.second].push_back(obstacle1);

        // Obstáculo 2 en (60, 60)
        ObjectDTO obstacle2(ObjectType::OBSTACLE, {90, 0}, 10, 10);
...
public:
    ...
    bool move(const Direction& direction, const uint16_t& id) {
        /*
    ...
    void add_player(uint16_t id) {
        ObjectDTO player(ObjectType::PLAYER, {10, 10}, PLAYER_SIZE, PLAYER_SIZE, id);
